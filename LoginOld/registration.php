...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        $sql = "SELECT id FROM officials WHERE username = ?";
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err)){
		...
		$sql = "INSERT INTO enforcers (username, password) VALUES (?, ?)";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
        ...
        $sql = "INSERT INTO officials (username, password) VALUES (?, ?)";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ss", $param_username, $param_password);
        ...
        echo "Please select your Barangay Position, Thanks.";
    }
...
<head>
     ...
     <title>Enforcer Login Panel</title>
...
<body>
	...
	<img src="man.jpg">
	  ...
	  <div class="contentBx">
	  <div class="formBX">
		  <h2>Barangay Enforcer Registration</h2>
